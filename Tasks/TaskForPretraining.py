...
#from model import BertForPrdeletrainingMo
from paddlenlp.transformers import BertForPretraining, BertModel, BertTokenizer
...
class ModelConfig:
    def __init__(self):
        ...
        self.batch_size = 32
        self.max_sen_len = 128  # 为None时则采用每个batch中最长的样本对该batch中的样本进行padding
        self.pad_index = 0
...
def train(config:ModelConfig):
    ...
    paddle.disable_static()
    base_model = BertModel(vocab_size=config.bert_config.vocab_size, type_vocab_size=2)
    model = BertForPretraining(base_model)
    last_epoch = -1
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (b_token_ids, b_segs, b_mask, b_mlm_label, b_nsp_label) in enumerate(train_iter()):
            ...
            # for input_id in (b_token_ids, b_segs, b_mask, b_mlm_label, b_nsp_label):
            ...
            loss, mlm_logits, nsp_logits = model(input_ids=b_token_ids,
                                                 ...
                                                 labels=b_mlm_label,
                                                 next_sentence_label=b_nsp_label)
            #print(loss, mlm_logits, nsp_logits)
            #optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            #scheduler.step()
            ...
            if idx % 20 == 0:
                ...
                # config.writer.add_scalar('Training/Learning Rate', config.learning_rate)#scheduler.get_last_lr()[0], scheduler.last_epoch)
        ...
        if (epoch + 1) % config.model_val_per_epoch == 0:
            ...
            torch.save({'last_epoch': epoch,#scheduler.last_epoch,
...
def accuracy(mlm_logits, nsp_logits, mlm_labels, nsp_label, PAD_IDX):
    ...
    mlm_pred = mlm_logits.argmax(axis=2)
    ...
    mlm_acc = mlm_pred.equal(mlm_true)  # 计算预测值与正确值比较的情况，得到预测正确的个数（此时还包括有mask位置）
    mask = torch.logical_not(mlm_true.equal(PAD_IDX))  # 找到真实标签中，mask位置的信息。 mask位置为FALSE，非mask位置为TRUE
    ...
    # print("mlm_acc")
    # print(mlm_acc)
    nsp_label = paddle.transpose(nsp_label, (1, 0)).reshape((32,))
    nsp_correct = (nsp_logits.argmax(1) == nsp_label).sum()

    # print("nsp_logits")
    # print(nsp_logits.argmax(1))
    ...
    with torch.no_grad():
        ...
        for idx, (b_token_ids, b_segs, b_mask, b_mlm_label, b_nsp_label) in enumerate(data_iter()):
            ...
            mlm_logits, nsp_logits = model(input_ids=b_token_ids,
                                           attention_mask=b_mask,
                                           token_type_ids=b_segs)
            print(mlm_logits, nsp_logits, b_mlm_label, b_nsp_label, PAD_IDX)
...
if __name__ == '__main__':
    ...
    sentences_2 = ["十年生死两茫茫。不思量。自难忘。千里孤坟，无处话凄凉。",
                   "红酥手。黄藤酒。满园春色宫墙柳。"]
    inference(config, sentences_2, masked=False, language='en',random_state=2022)
