...
def pad_sequence(sequences, batch_first=False, max_len=None, padding_value=0):
    ...
    for tensor in sequences:
        if tensor.shape[0] < max_len:
            tensor = paddle.concat([tensor, paddle.to_tensor([padding_value] * (max_len - tensor.shape[0]))], axis=0)
        else:
            tensor = tensor[:max_len]
        out_tensors.append(tensor)
    out_tensors = paddle.stack(out_tensors, axis=1)
    if batch_first:
...
class LoadSingleSentenceClassificationDataset:
    ...
    def data_process(self, filepath, postfix='cache'):
        ...
        for raw in tqdm(raw_iter, ncols=80):
            ...
            tmp += [self.SEP_IDX]
            tensor_ = paddle.to_tensor(tmp, dtype=paddle.int64)
            l = paddle.tensor(int(l), dtype=paddle.int54)
    ...
    def generate_batch(self, data_batch):
        ...
        batch_label = paddle.to_tensor(batch_label, dtype=paddle.int64)
...
class LoadPairSentenceClassificationDataset(LoadSingleSentenceClassificationDataset):
    ...
    def data_process(self, filepath, postfix='cache'):
        ...
        for raw in tqdm(raw_iter, ncols=80):
            ...
            segs = paddle.to_tensor(seg1 + seg2, dtype=paddle.int64)
            tensor_ = paddle.to_tensor(tmp, dtype=paddle.int64)
            l = paddle.tensor(int(l), dtype=paddle.int64)
            max_len = max(max_len, tensor_.size(0))
...
class LoadMultipleChoiceDataset(LoadSingleSentenceClassificationDataset):
    ...
    def generate_batch(self, data_batch):
        ...
        def get_seq(q, a):
            ...
            return paddle.to_tensor(seq + [self.SEP_IDX], dtype=paddle.int64)
        ...
        for item in data_batch:
            ...
            tmp_seg = [paddle.to_tensor(seg0, dtype=paddle.int64),
                       paddle.to_tensor(seg1, dtype=paddle.int64),
                       paddle.to_tensor(seg2, dtype=paddle.int64),
                       paddle.to_tensor(seg3, dtype=paddle.int64)]
        ...
        for example in tqdm(examples, ncols=80, desc="正在遍历每个问题（样本）"):
            ...
            if context_ids_len > rest_len:  # 长度超过max_sen_len,需要进行滑动窗口
                ...
                while True:
                    ...
                    input_ids = paddle.to_tensor(question_ids + tmp_context_ids + [self.SEP_IDX])
                ...
                input_ids = paddle.to_tensor(question_ids + context_ids + [self.SEP_IDX])
...
class LoadChineseNERDataset(LoadSingleSentenceClassificationDataset):
    ...
    def data_process(self, filepath, postfix='cache'):
        ...
        for raw in tqdm(raw_iter, ncols=80):
            ...
            if len(line) == 1:  # 表示得到一个完整的token id样本
                ...
                token_ids = paddle.to_tensor([self.CLS_IDX] + tmp_token_ids +
                                         [self.SEP_IDX], dtype=paddle.long)
                labels = paddle.to_tensor([self.IGNORE_IDX] + tmp_label +
                                      [self.IGNORE_IDX], dtype=paddle.long)
    ...
    def make_inference_samples(self, sentences):
        ...
        for sen in sentences:
            ...
            label = [-1] * len(tokens)
            token_ids = paddle.to_tensor([self.CLS_IDX] + tokens + [self.SEP_IDX], dtype=paddle.long)
            labels = paddle.to_tensor([self.IGNORE_IDX] + label + [self.IGNORE_IDX], dtype=paddle.long)
