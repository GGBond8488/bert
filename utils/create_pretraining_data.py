...
class LoadBertPretrainingDataset(object):
    ...
    def data_process(self, filepath, postfix='cache'):
        ...
        for paragraph in tqdm(paragraphs, ncols=80, desc=desc):  # 遍历每个
            for i in range(len(paragraph) - 1):  # 遍历一个段落中的每一句话
                ...
                segs = paddle.to_tensor(segs, dtype=paddle.int64)
                logging.debug(f" ## Mask之前token ids:{token_ids}")
                logging.debug(f" ##      segment ids:{segs.tolist()},序列长度为 {len(segs)}")
                nsp_lable = paddle.to_tensor(int(is_next), dtype=paddle.int64)
                mlm_input_tokens_id, mlm_label = self.get_masked_sample(token_ids)
                token_ids = paddle.to_tensor(mlm_input_tokens_id, dtype=paddle.int64)
                mlm_label = paddle.to_tensor(mlm_label, dtype=paddle.int64)
                max_len = max(max_len, token_ids.size)
    ...
    def generate_batch(self, data_batch):
        ...
        b_token_ids = paddle.transpose(b_token_ids, (1, 0))
        ...
        b_segs = pad_sequence(b_segs,  # [batch_size,max_len]
                              ...
                              batch_first=False,
                              max_len=self.max_sen_len)
        b_segs = paddle.transpose(b_segs, (1, 0))
        ...
        b_mlm_label = pad_sequence(b_mlm_label,  # [batch_size,max_len]
                                   ...
                                   batch_first=False,
                                   max_len=self.max_sen_len)
        b_mlm_label = paddle.transpose(b_mlm_label, (1, 0))
        # b_mlm_label:  [batch_size, src_len,]

        #b_mask = (b_token_ids == self.PAD_IDX).transpose(0, 1)
        b_mask = paddle.transpose(b_token_ids == self.PAD_IDX, (0, 1))
        # b_mask: [batch_size, max_len, ]

        b_nsp_label = torch.to_tensor(b_nsp_label, dtype=torch.long)
        #b_nsp_label = paddle.transpose(b_nsp_label, (1, 0))
    ...
    def make_inference_samples(self, sentences=None, masked=False, language='en', random_state=None):
        ...
        for sen in sentences:
            ...
            input_tokens_ids.append(paddle.to_tensor(token_ids, dtype=paddle.long))
